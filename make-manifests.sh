...
mk_kubernetes_manifest() {
    ...
    do
        ...
        cat ../microservices-demo/kubernetes-manifests/${svc}.yaml >> ${CLUSTER_NAME}.yaml
...
mk_istio_manifests() {
	...
	cat $HOME/istio-1.2.6/install/kubernetes/helm/istio-init/files/crd-* >> manifests/istio-generic.yaml
	helm template $HOME/istio-1.2.6/install/kubernetes/helm/istio --name istio --namespace istio-system -f istio/values/gateway-multicluster.yaml >> manifests/istio-generic.yaml

	# Override for AWS
	print_license_header manifests/istio-aws
	print_autogenerated_warning manifests/istio-aws
	cat $HOME/istio-1.2.6/install/kubernetes/helm/istio-init/files/crd-* >> manifests/istio-aws.yaml
	helm template $HOME/istio-1.2.6/install/kubernetes/helm/istio --name istio --namespace istio-system -f istio/values/gateway-multicluster.yaml -f istio/values/override-aws.yaml >> manifests/istio-aws.yaml
}

CLUSTER_NAME="manifests/azure"
AZURE_SVCS="emailservice paymentservice shippingservice"
#mk_kubernetes_manifest $AZURE_SVCS

CLUSTER_NAME="manifests/google"
GCP_SVCS="frontend productcatalogservice recommendationservice"
	
#mk_kubernetes_manifest $GCP_SVCS

CLUSTER_NAME="manifests/onprem"
ONPREM_SVCS="adservice checkoutservice currencyservice cartservice currencyservice redis"
